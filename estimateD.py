...
def estimate_d(I, J, x, y, window_size, max_iterations=50, min_error=0):
    ...
    Ig, Jg, Jgdx, Jgdy = regularized_values(I, J, 10, 1)

    Jg_og = np.copy(Jg)
    Jgdx_og = np.copy(Jgdx)
    Jgdy_og = np.copy(Jgdy)
    ...
    for tmp in range(max_iterations):
        ...
        Jg = Jg_interpol(np.arange(dtot[1], im_height + dtot[1]), np.arange(dtot[0], im_width + dtot[0]))

        Jgdx_interpol = interpolate(Jgdx_og, 0, 0, im_height, im_width)
        Jgdx = Jgdx_interpol(np.arange(dtot[1], im_height + dtot[1]), np.arange(dtot[0], im_width + dtot[0]))

        Jgdy_interpol = interpolate(Jgdy_og, 0, 0, im_height, im_width)
        Jgdy = Jgdy_interpol(np.arange(dtot[1], im_height + dtot[1]), np.arange(dtot[0], im_width + dtot[0]))
    return dtot
